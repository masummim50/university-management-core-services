...
const getAllFaculty = catchAsync(async (req: Request, res: Response) => {
  ...
  const { data, meta } = await AcademicFacultyServices.getAllFromDb(
    filters,
    options
  );
  sendResponse(
...
const insertIntoDb = catchAsync(async (req: Request, res: Response) => {
  const data = await AcademicFacultyServices.insertIntoDb(req.body);
...
const getFacultyById = catchAsync(async (req: Request, res: Response) => {
  const data = await AcademicFacultyServices.getById(req.params.id);
  sendResponse(
    res,
    httpStatus.OK,
    true,
